...
class Robot:
    ...
    def measure(self, markers, idx_list):
        ...
        if (len(measurements)!=0):
            markers_bff = np.concatenate(measurements, axis=1) # Modified HERE
        return markers_bff
    ...
    def covariance_drive(self, drive_meas):
        ...
        if ang_vel==0 :
            ...
            Jac2[0,1] = -lin_vel*(-np.sin(th) + np.sin(th2))/ang_vel**2
            Jac2[1,0] = (np.cos(th) - np.cos(th2))/ang_vel
            Jac2[1,1] = -lin_vel*(np.cos(th) - np.cos(th2))/ang_vel**2
            Jac2[2,1] = dt
